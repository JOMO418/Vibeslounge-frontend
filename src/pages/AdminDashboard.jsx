// ============================================================================
// FIXED client/src/pages/AdminDashboard.jsx
// ============================================================================
...
import { Loader2, LogOut, TrendingUp, DollarSign, ShoppingCart, Package, Download, Trash2, UserPlus, Edit } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'hsl(0 0% 4%)' }}>
      {/* Top Bar */}
      <header className="sticky top-0 z-50 px-6 py-4" style={{ backgroundColor: 'hsl(0 0% 10%)', borderBottom: '1px solid hsl(0 0% 17%)' }}>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold" style={{ color: 'hsl(0 0% 98%)' }}>Admin Dashboard</h1>
            <p className="text-sm" style={{ color: 'hsl(0 0% 54%)' }}>{user?.email}</p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <p className="text-xs uppercase tracking-wide mb-1" style={{ color: 'hsl(0 0% 54%)' }}>TODAY'S PROFIT</p>
              <div className="flex items-center gap-2">
                <TrendingUp className="h-6 w-6" style={{ color: 'hsl(142 71% 45%)' }} />
                <p className="text-4xl md:text-5xl font-bold" style={{ color: 'hsl(45 93% 47%)' }}>{formatKES(todaysProfit)}</p>
              </div>
            </div>
            <button 
              onClick={handleLogout}
              className="flex items-center gap-2 px-4 py-2 rounded-lg transition"
              style={{ border: '1px solid hsl(0 0% 17%)', backgroundColor: 'transparent', color: 'hsl(0 0% 98%)' }}
                ...
                {staffMembers && staffMembers.length > 0 ? (
                  staffMembers.map((staff, idx) => (
                    <tr 
                      ...
                      onMouseLeave={(e) => e.currentTarget.style.backgroundColor = idx % 2 === 0 ? 'hsl(0 0% 10%)' : 'hsl(0 0% 10% / 0.5)'}
                    >
                      <td className="py-3 px-4 font-semibold" style={{ color: 'hsl(0 0% 98%)' }}>{staff.email}</td>
                      <td className="py-3 px-4">
                        <span 
                      ...
                      <td className="py-3 px-4 text-sm" style={{ color: 'hsl(0 0% 54%)' }}>
                        {new Date(staff.createdAt).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', year: 'numeric' })}
                      </td>
                      <td className="py-3 px-4 text-center">
                        {staff._id !== user?._id && (
                          <button 
